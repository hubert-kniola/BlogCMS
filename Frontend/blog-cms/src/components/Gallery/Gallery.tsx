...
const img1 = 'https://c8.alamy.com/comp/HNPYX1/vertical-view-of-an-old-town-in-europe-in-beautiful-golden-evening-HNPYX1.jpg'
const imgs = [img, img,img,img1, img, img1, img, img1, img, img1, img, img1, img, img1]
      ...
      <div className={BEM(css.gallery, css.container)}>
        <div className={BEM(css.gallery, css.image)}>
          <div className={BEM(css.gallery, css.active)}>
            <img src={imgs[active]} />
            <div className={BEM(css.gallery, css.image, css.left)} onClick={() => previous()}>
              <ArrowLeftCircleIco />
            </div>
            <div className={BEM(css.gallery, css.image, css.right)} onClick={() => next()}>
              <ArrowRigthCicleIco />
            </div>
          </div>
        </div>
        <div className={BEM(css.gallery, css.grid)}>
          {imgs.map((item, idx) => {
            return (
              <div key={idx}>
                <img src={item} onClick={() => setActive(idx)} />
              </div>
            );
          })}
        </div>
