...
const image =
  "https://img.freepik.com/free-photo/horizontal-shot-surprised-unshaven-young-male-points-down-opens-mouth-widely-sees-something-stunning-floor-wears-stylish-shirt-isolated-white-wall-people-emotions-concept_273609-16521.jpg?w=2000";

export const Pomodoro = ({ premiere, title }: IPomodoro) => {
  ...
  const getPremiereDate = () => {
    return `${premiere.toLocaleDateString("pl-Pl")} ${formatTime(
      premiere.getHours()
    )}:${formatTime(premiere.getMinutes())}`;
  };
  ...
  return (
    <div className={BEM(css.pomodoro)}>
      <div className={BEM(css.pomodoro, css.title)}>
        ...
        <p>{title && title}</p>
        {!isTime && premiere && <h1>Premiera: {getPremiereDate()}</h1>}
      </div>
